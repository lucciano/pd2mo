%DONE:agregar ejemplo jerarquico de un nivel : ejemplo con powerdevs.
%presentar los bloques vectoriales que se usan, mostrar los modelos que se usa (p12 del paper de devs vectorial). 3.4 de esta doc, agregar los modelos en modelica y su relacion.

%trabajos futuros,
%	desarrollar mas modelos (modelica)
%	integrarlo con powerdevs
%   extension de las plantillas en modelos vectoriales para mas flexibilidad
%	nuevo parser
%   logs...(quitar debug), errores fatales....
%   depende del causalizador
...
Para conectar bloques vectoriales y bloques escalares es necesarios bloques que hagan de interfaz entre los dos formalismo.

\begin{itemize}
 \item Index Shift: El modelo más simple es el Index Shift. Cuando se recibe un evento con el valor $(x,i)$, envia un evento de salida $(x, i+sh)$, donde $sh$ es un parametro entero.
