...
%where:
%g(u, v) = (max(u − U thres , 0)) 2 − (max (u − v − U thres , 0)) 2
%(10)
%We used the set of parameters given in the article: Υ = 100 (which results in a very
%stiff system), U thres = 1, and U op = 5. The initial conditions are, as in the given reference,
%ω j = 6.247 · 10 −3 for odd values of j and ω j = 5 for even values of j. The input is a periodic
%trapezoid signal, with parameters V up = 5V , V low = 0V , T down = 10, T up = 5, T rise = 5, and
%T f all = 2.

	la cual es representada por el modelo PowerDEVS de la Figura \ref{model:inverters}.
