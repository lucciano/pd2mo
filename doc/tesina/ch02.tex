...
\todo[inline]{Cita los algoritmos DASSL, RK, DOPRI}
...
\section{Modelica}
    ...
    \todo[inline]{Ojo pareciera que OM no tiene editor y sí lo tiene}
...
\end{listing}
    \todo[inline]{Acá yo pondría una version del modelo que no esté ordenada, por ejemplo \texttt{0=-der(x)+ ... } }
    ...
    \todo[inline]{Acá esto queda claro mostrando el modelo no ordenado como antes...si ponés 0=... no estás asignando el cero claramente}
...
\todo[inline]{Mencionar que $mu$-Modelica es un subconjunto de Modelica!}
...
\todo[inline]{Arreglos unidimensionales}
     Arreglos están permitidos. Indices en los arreglos dentro de cláusulas \texttt{for} están restringidos a la forma $\alpha \cdot i + \beta$, 
...
\todo[inline]{Acá también se puede asignar solo una variable de la forma $a_2 = g(a_1)$}
...
\todo[inline]{Explicar que estas restricciones son para que la herramienta que vas a presentar luego pueda detectar la estructura fácilmente}
\section{Stand–Alone QSS solver}
\todo[inline]{Acá también quitar la palabra desperdician}
	...
	Estas desventajas motivó el desarrollo del \emph{Stand–Alone QSS solver}\cite{Ber12}, implementados como un conjunto de módulos en lenguaje C. Este implementa 
	toda la familia de métodos QSS y permite que los modelos contengan discontinuidades de tiempo y estado.

\todo[inline]{Más que una dificultad es un requerimiento de los métodos}
...
\todo[inline]{Ojo acá no hace falta que las variables de estado estén en un arreglo. En primeras versiones del solver sí pero ahora no. Quizás es mejor hacerlo lo más parecido al ejemplo inicial}
	\begin{listing}[H]    
		\caption{LotkaVolterra.mo}
...
\todo[inline]{Acá podés remarcar que en esta caso las ecuaciones sí deben ser de la forma var=f()} 
