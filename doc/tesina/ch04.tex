...
\todo[inline]{Yo haría un dibujito típico de pipeline de qué orden se corren las cosas. Parsing->flat->mda->if->prod. También los describiría en el orden que se aplican}
\smartdiagramset{
uniform color list=gray!60!black for 5 items,
back arrow disabled=true,
}
\smartdiagram[flow diagram:horizontal]{Parsing,Flat,mda,if,prod}
...
\todo[inline]{La linea 15 poné algo que parezca pseudo codigo y luego lo explicás en el texto...queda muy largo}
\begin{algorithm}[H]
\begin{algorithmic}[1]
...
\section{Transformaciones para $\mu$-Modelica} \label{sec:transform}
	...
	Estas transformaciones son necesarias dado que al momento de realizar este trabajo 
\todo[inline]{yo aclararía que ciertas expresiones no son soportados}
...
\todo[inline]{Ojo que esto lo que hace es expandir los arreglos bidimensionales en varias variables unidimensionales. Luego para que sea consistente ajusta las aparciones del arreglo anterior a los nuevos arreglos}
	  \begin{itemize}
...
\todo[inline]{aclará que el $*$ entre vectores es el prod interno, por eso lo podés reemplzar}
