...
\end{listing}
	...
	modelo (posición $0$).
        ...
        Tanto EIC y EOC siguen el mismo patrón excepto que replazan el modelo de los puertos de salida y entrada respectivamente por $0$. Por ejemplo el par 
...
\section{Modelos Atómicos}
	...
	modelo atómico con anticipación y aderir a las convenciones que describiremos en esta sección, por supuesto esta actividad requiere el conocimiento del modelo y de Modelica.
...
\end{itemize}
	...
	salida \texttt{Real y[1]}. Si bien tiene cuatro parametros el unico utlizable dentro del modelo Modelica es la entrada 4, \texttt{p[4]}, el cual es el utilizado como valor
	inicial de ingración.
...
\begin{figure}[H]
  ...
  \makebox[\textwidth][c]{%
        ...
        \caption{Modelo convertidor Buck de potencia, jerarquía original (izquierda) y aplanada (derecha), jutno al nombre, esta la posición que ocupan dentro del modelo acoplado que los contiene.}
...
\end{figure}
	...
	En el lado derecho de la \ref{fig:coupled-tree}, se pueden ver como quedaron l
