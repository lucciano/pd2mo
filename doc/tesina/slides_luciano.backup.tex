...
\scalebox{0.50}{%
        \begin{tikzpicture}[%
          ...
          \node {Root-Coordinator}
            ...
            child { node (C) {GNUPlot0   (8)}}
            child { node (B) {Hysteresis0(9)}}
            child { node (B) {Switch1    (10)}}
            child { node (A) {WSum0      (11)}}
            child { node {WSum1      (12)}};
	    \draw [->] (A.east) edge[bend  right=90] (B.east) (B.east) edge[bend right=90] (C.east) (C.east) edge[bend right=90] (D.east);
...
\begin{frame}{Resultados - Convertidor de Voltaje}
 \includegraphics[width=\linewidth]{output/buck_disk/buck_disk-all}
\end{frame}
